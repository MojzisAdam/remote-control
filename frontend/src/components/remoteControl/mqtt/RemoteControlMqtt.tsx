...
const RemoteControlMqtt: React.FC<RemoteControlMqttProps> = ({ device, onDataReceived, onVersionUpdate }) => {
	...
	useEffect(() => {
		const setupSubscription = async () => {
			...
			try {
				const topic = `/amit_cim/publish/${device.id}/#`;
		...
		const sendInitialMessage = async () => {
			...
			try {
				const sendTopic = `/amit_cim/subscribe/${device.id}/w`;
	...
	const publishMqtt = useCallback(
		async (register: number, value: number) => {
			...
			const topic = ["reg_64", "reg_65", "reg_71", "reg_68", "reg_75", "reg_76", "reg_77", "reg_78", "reg_33", "reg_35"].includes(name)
				? `/amit_cim/subscribe/${device.id}/1`
				: `/amit_cim/subscribe/${device.id}/2`;
	...
	const publishMqttExtended = useCallback(
		async (register: number, value: number) => {
			const topic = `/amit_cim/subscribe/${device.id}/change`;
			const message = JSON.stringify({
				reg_change: 1,
				reg_name: register,
				reg_value: value,
	...
	useEffect(() => {
		...
		const setupExtendedSubscription = async () => {
			try {
				...
				const topic = `/amit_cim/publish_s/${device.id}/params`;
				await subscribeToTopic(topic);
				isExtendedSubscribed.current = true;

				const sendTopic = `/amit_cim/subscribe/${device.id}/ws`;
				await publishMessage(sendTopic, JSON.stringify({ send: 1 }), { qos: 1 });
			} catch (err) {
	...
	useEffect(() => {
		if ((deviceConnectionStatus === "disconnected" || deviceConnectionStatus === "offline") && isExtendedSubscribed.current) {
			if (client && connectionStatus === "connected") {
				client.unsubscribe(`/amit_cim/publish_s/${device.id}/params`);
			}
			isExtendedSubscribed.current = false;
		}
