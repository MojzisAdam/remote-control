...
import { DeviceData } from "@/components/remoteControl/mqtt/daitsu/RemoteControl";
import { getBit, getLowerByte } from "@/utils/utils";
		...
		>
			...
			<div className="outdoor-temp">
				<p className="flex items-center space-x-1">
					...
					<span>{getTemperatureDisplay(deviceData.reg_107)}</span>
				</p>
			</div>
			...
			<div className="home-image space-y-2 min-[900px]:space-y-4">
				<HomeSvg />
				<p>
					<span className={getBit(deviceData.reg_129 ?? 0, 5) === 1 ? "home_tuvto-on" : "home_tuvto-off"}></span>
					<RadiatorIcon />
					<span
						className="want-temp-home"
						data-descr={`${(() => {
							if (deviceData.reg_136 == 255) {
								return `${getLowerByte(deviceData.reg_2 ?? 0)} °C`;
							}
							return `${deviceData.reg_136} °C`;
						})()}`}
					>
						{(() => {
							return getTemperatureDisplay(deviceData.reg_110);
						})()}
					</span>
				</p>

				{/* Additional temperature displays */}
				{getBit(deviceData.reg_210 ?? 0, 15) === 1 && (
					<p>
						<span className={getBit(deviceData.reg_129 ?? 0, 4) === 1 ? "home_tuvto-on" : "home_tuvto-off"}></span>
						<FaucetIcon />
						<span
							className="want-temp-home"
							data-descr={`${deviceData.reg_4} °C`}
						>
							{getTemperatureDisplay(deviceData.reg_115)}
						</span>
					</p>
				)}
			...
			<div className="outdoor-unit">
				<span className={getBit(deviceData.reg_129 ?? 0, 13) === 0 ? "outdoor-unit-off" : "outdoor-unit-on"}></span>
				<OutdoorUnitIcon />
				<p>{deviceData.reg_100} Hz</p>
				{(() => {
					if (deviceData.reg_101 === 3) {
						return (
							<p className="stav_venk">
								<HeatIcon />
							</p>
						);
					} else if (deviceData.reg_101 === 2) {
						return (
							<p className="stav_venk">
								<ColdIcon />
							</p>
						);
					} else if (getBit(deviceData.reg_129 ?? 0, 15) === 1) {
						return (
							<p className="stav_venk">
								<DefrostIcon />
							</p>
